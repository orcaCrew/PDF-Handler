  ...
  useEffect(() => {
    const updatePreviewWidth = () => {
      if (previewRef.current) {
        ...
        setPreviewWidth(Math.min(containerWidth - 40, 800)); // 패딩 고려
      }
    };
              ...
              {file ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* 선택된 페이지 미리보기 영역 */}
                    <div className="space-y-4 lg:col-span-2" ref={previewRef}>
                      <h3 className="text-lg font-semibold text-gray-800">선택된 페이지</h3>
                      {pageNumber ? (
                        <div className="flex justify-center">
                          <div className="border border-gray-300 rounded-lg shadow-lg bg-white w-full">
                            <Document
                              file={file}
                              onLoadSuccess={onDocumentLoadSuccess}
                              onLoadError={onDocumentLoadError}
