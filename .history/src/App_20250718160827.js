...
function App() {
  ...
  const [displayedPages, setDisplayedPages] = useState(3); // 처음에 3페이지만 표시
  ...
  const onFileChange = (event) => {
    ...
    if (files && files[0]) {
      ...
      setDisplayedPages(3); // 새로운 파일이 로드될 때 displayedPages 초기화
    }
              ...
              {file ? (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    ...
                    >
                      {numPages && Array.from(new Array(Math.min(displayedPages, numPages)), (el, index) => (
                        <div
                          key={`page_${index + 1}`}
                    ...
                    {numPages && displayedPages < numPages && (
                      <div className="text-center pt-4">
                        <button
                          onClick={() => setDisplayedPages(prev => Math.min(prev + 3, numPages))}
                          className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
                        >
                          더 보기 ({displayedPages}/{numPages})
                        </button>
                      </div>
                    )}
                  </div>
